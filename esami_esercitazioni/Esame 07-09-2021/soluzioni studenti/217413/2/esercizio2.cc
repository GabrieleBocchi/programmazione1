...
void extract_(const char source[]);
void extract_aux(const char source[], int i);
...
char * extract(char source[]) {
  // MR: Manca body!!
}

void extract_(const char source[]) {
  extract_aux(source, 0);
}
void extract__aux(const char source[], int i) {
  if (source[i] == '@') {
    // cout << '@';
    // MR: Non considero il fatto che non compilava!!!
    extract__aux(source, i+1);
  }
}
