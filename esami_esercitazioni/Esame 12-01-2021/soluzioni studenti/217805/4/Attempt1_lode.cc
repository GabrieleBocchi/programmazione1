...
void stampatutti(const tree * t, char * autore) {
  if (t != NULL) {
    int ris = comparaautore(autore, t->info);
    if (ris < 0) {
      // L'autore che cerco viene prima, nell'alfabeto, rispetto all'autore del nodo attuale
      stampatutti(t->left, autore);
    } else if (ris > 0) {
      // L'autore che cerco viene dopo, nell'alfabeto, rispetto all'autore del nodo attuale
      stampatutti(t->right, autore);
    } else {
      stampalibro(t->info);
      // Continua a stampare i libri dello stesso autore
      stampatutti(t->left, autore);
    }
  }
}
