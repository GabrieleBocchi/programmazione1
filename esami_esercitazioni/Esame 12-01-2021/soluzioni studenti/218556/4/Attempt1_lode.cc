...
void stampatutti(tree *libri, char *autore)
{
  // MR: Cosa succede se libri == NULL?
  if (libri->left != NULL)
  {
    stampatutti(libri->left, autore);
  }
  if (comparaautore(autore, libri->info) == 0)
  {
    stampalibro(libri->info);
  }

  if (libri->right != NULL)
  {
    stampatutti(libri->right, autore);
  }
}
