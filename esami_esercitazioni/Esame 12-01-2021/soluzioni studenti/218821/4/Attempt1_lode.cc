...
//////////////////////////////////////////
void stampatutti(const tree *t, char *a) {
    if (t != (tree *)NULL) {
        if (comparaautore(a, t->info) == 0) {
            stampalibro(t->info);
            stampatutti(t->right, a);
	    // MR: Qui doveva fare ricorsione a sinistra!
        } else {
            if (comparaautore(a, t->info) < 0) {
                stampatutti(t->left, a);
            } else if (comparaautore(a, t->info) > 0) {
                stampatutti(t->right, a);
            }
        }
    }
